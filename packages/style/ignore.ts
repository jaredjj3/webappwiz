...
const MARKER = /style-ignore(-file)?\s+([\w-]+)\s*:\s*\S/;

/**
 * Tells you which lines of a file a `style-ignore` comment excuses from the
 * rule `id`, given the file's lines:
 *
 * ```ts
 * // style-ignore no-em-dashes: quoting a spec that spells them out
 * ```
 *
