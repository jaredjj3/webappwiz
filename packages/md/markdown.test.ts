...
describe("Markdown", () => {
	describe("fields", () => {
		...
		it("unquotes quoted values, as YAML would", () => {
	...
	describe("sections", () => {
		...
		it("stops the lead at the first subsection", () => {
			const good = Markdown.parse(doc).section("Good");

			expect(good.lead).toBe(
				"One class, one file.\n\n```ts\nclass Foo {}\n```",
			);
		});
	...
	describe("codeBlocks", () => {
		it("collects fences with and without a language", () => {
			const md = Markdown.parse(doc);
		...
		it("scopes to the lead when asked", () => {
