...
describe("changed", () => {
	...
	it("names files the way a rule's glob does, relative to the directory", async () => {
		ps.setCaptureOutput("src/b.ts\nsrc/a.ts\n", "");

		expect(
			(await changed("/p", "main", { ps })).map((file) => file.path),
		).toEqual(["src/a.ts", "src/b.ts"]);
	});

	it("counts a file once when it is both changed and untracked, as changed", async () => {
		ps.setCaptureOutput("src/a.ts\n", "");

		expect(await changed("/p", "main", { ps })).toEqual([
			{ path: "src/a.ts", added: false },
		]);
	});
