...
test("ports are deterministic and inside the configured range", async () => {
	const f = await fixture();

	expect(f.arbor.store.portFor("alpha")).toBe(f.arbor.store.portFor("alpha"));
	for (const task of ["alpha", "beta", "a-longer-task-name", "z"]) {
		expect(f.arbor.store.portFor(task)).toBeGreaterThanOrEqual(3100);
		expect(f.arbor.store.portFor(task)).toBeLessThanOrEqual(3199);
	}
	await f.cleanup();
});
