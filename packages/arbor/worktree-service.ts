...
export class WorktreeService {
	...
	// rule-ignore classes-over-function-exports: Git is the only way arbor speaks git and a second implementation is not coming, so there is no interface to write; tests run the real Git against a throwaway repo rather than faking twenty methods
	constructor(
		readonly git: Git,
