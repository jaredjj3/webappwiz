...
describe("cost", () => {
	...
	it("reads back what a run recorded", async () => {
		await calibrate(fs, "/p", "haiku", 2.5);

		expect(await overheads(fs, "/p")).toEqual({ haiku: 2.5 });
	});

	it("keeps each agent's measurement when another is recorded", async () => {
		await calibrate(fs, "/p", "haiku", 2.5);
		await calibrate(fs, "/p", "opus", 1.8);

		expect(await overheads(fs, "/p")).toEqual({ haiku: 2.5, opus: 1.8 });
	});

	it("replaces an agent's earlier measurement with the newer one", async () => {
		await calibrate(fs, "/p", "haiku", 2.5);
		await calibrate(fs, "/p", "haiku", 1.9);

		expect(await overheads(fs, "/p")).toEqual({ haiku: 1.9 });
	});
	...
	it("drops a recorded overhead that is not a positive number", async () => {
		...
		expect(await overheads(fs, "/p")).toEqual({ opus: 1.8 });
	});
});
