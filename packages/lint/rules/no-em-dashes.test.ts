...
describe("no-em-dashes", () => {
	it("flags an em dash where it sits", () => {
		expect(noEmDashes("stderr \u2014 keep it parseable")).toEqual([
			{
				line: 1,
				column: 8,
	...
	it("flags an en dash between words", () => {
		const findings = noEmDashes("before \u2013 after");
	...
	it("keeps the en dash of a digit range", () => {
		expect(noEmDashes("retry in 5\u201310 seconds")).toEqual([]);
	});

	it("reports the line a later dash sits on", () => {
		expect(noEmDashes("clean\nclean\nnot \u2014 clean")).toEqual([
