...
const MARKER = /lint-ignore(-file)?\s+([\w-]+)\s*:\s*\S/;

/**
 * Tells you which lines of a file a `lint-ignore` comment excuses from the
 * rule `id`, given the file's lines:
 *
 * ```ts
 * // lint-ignore no-em-dashes: quoting a spec that spells them out
 * ```
 *
 ...
 * when it sits above one. `lint-ignore-file` covers the file instead.
 */
