...
export class SchemaObject<
	P extends Record<string, Schema<unknown>>,
> extends SchemaBase<{ [K in keyof P]: Infer<P[K]> }> {
	constructor(private props: P) {
		super();
	}

	parse(value: unknown): { [K in keyof P]: Infer<P[K]> } {
		...
		for (const [key, schema] of Object.entries(this.props)) {
			try {
				out[key] = schema.parse((value as Record<string, unknown>)[key]);
			} catch (e) {
	...
	coerce(raw: string): { [K in keyof P]: Infer<P[K]> } {
